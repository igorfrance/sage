...
namespace Sage.Views
{
	...
	{
		private static readonly ILog log = LogManager.GetLogger(typeof(ViewModel).FullName);
		...
		public ModuleResultStatus ResultStatus
		{
			get
			{
				...
				foreach (var type in this.ModuleResults.Keys)
				{
					foreach (ModuleResult result in this.ModuleResults[type])
					{
		...
		private void AddModuleType(string moduleKey)
		{
			...
			modules.Add(moduleKey, moduleConfig);

			foreach (string key in moduleConfig.ModuleDependencies.Where(n => !modules.ContainsKey(n)))
			{
				...
				modules.Add(key, config.Modules[key]);
			}
			...
			foreach (ModuleConfiguration module in temp)
			{
				...
				moduleResources.AddRange(module.Resources);
			}
