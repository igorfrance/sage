...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="*" mode="xmlroot">
		...
		<div class="xmltree">
			...
			&#160;
			<!-- 
				adding an invisible character to the result hacks the browser into 
				ignoring the whitespace between tags, thereby ensuring proper formatting
			-->
	...
	<xsl:template match="comment()" mode="xmltree">
		<xsl:param name="level" select="1"/>
		<xsl:param name="expandLevels" select="1"/>
		<div class="comment">
			<xsl:apply-templates select="." mode="xswitch">
				<xsl:with-param name="level" select="$level" />
				<xsl:with-param name="expandLevels" select="$expandLevels" />
	...
	<xsl:template match="node()" mode="xswitch">
		...
		<xsl:param name="expandLevels"/>
		<xsl:choose>
			<xsl:when test="parent::sage:model">
