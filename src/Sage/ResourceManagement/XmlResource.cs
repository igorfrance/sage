namespace Sage.ResourceManagement
{
	...
	public class XmlResource
	{
		...
		public XmlResource(string path, SageContext context)
		{
			...
			var config = ProjectConfiguration.Current;

			this.context = context;

			this.FilePath = path;
			this.Name = new ResourceName(path, config.Categories[context.Category]);
			this.SourceDirectory = Path.GetDirectoryName(path);

			if (UrlResolver.GetScheme(path) == "file")
			{
				this.TargetDirectory = Path.Combine(SourceDirectory, config.PathTemplates.GlobalizedDirectory);
			}
			else
			{
				string converted = path.ReplaceAll(convertScheme, "$1/$2");
				string expanded = Path.Combine(config.PathTemplates.GlobalizedDirectoryForNonFileResources, converted);
		...
		public DateTime DateLastModified
		{
			get
			{
				CategoryInfo category = ProjectConfiguration.Current.Categories[context.Category];
		...
		public CacheableXmlDocument Load(string locale)
		{
			...
			if (!config.AreResourcesPreGenerated)
			{
