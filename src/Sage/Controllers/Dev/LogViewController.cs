namespace Sage.Controllers.Dev
{
	...
	public class LogViewController : SageController
	{
		...
		[UrlRoute(Path = DashboardController.DevLog)]
		public ActionResult ViewLog()
		{
			if (!Context.IsDeveloperRequest)
				return PageNotFound();

			return View("view-complete-log");
		}
		...
		/// </summary>
		/// <returns>The result of processing this view.</returns>
		[UrlRoute(Path = "dev/log/{threadID}")]
		...
		private static List<string> GetThreadLogs(string threadID)
		{
		...
		private static XmlDocument CreateLogDocumentForThread(long threadID)
		{
			...
			List<string> logLines = GetThreadLogs(threadID.ToString());
			if (logLines != null)
			{
				Match match;
