...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="*" mode="tree">
		...
		<div class="element">
			...
			<span class="markup">&lt;</span><span class="nodeName element-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/></span>
			<xsl:apply-templates select="@*" mode="tree"/><xsl:apply-templates select="." mode="namespace"/>
	...
	<xsl:template match="*[node()]" mode="tree">
		...
		<div class="element">
			...
			<span class="markup">&lt;</span><span class="nodeName element-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/></span><xsl:if test="@*"><xsl:text> </xsl:text></xsl:if>
			<xsl:apply-templates select="@*" mode="tree"/><xsl:apply-templates select="." mode="namespace"/><span class="markup">></span>
			<div class="children">
				...
				<div>
					<span class="switch_space">&#160;</span>
					<span class="markup">&lt;/</span><span class="nodeName element-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/></span><span class="markup">></span>
				</div>
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template match="*[text() and not (comment() or processing-instruction())]" mode="tree">
		...
		<div class="element">
			<span class="switch_space">&#160;</span>
			<span class="markup">&lt;</span><span class="nodeName element-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/><xsl:if test="@*"><xsl:text> </xsl:text></xsl:if></span><xsl:apply-templates select="@*" mode="tree"/><xsl:apply-templates select="." mode="namespace"/><span class="markup">></span><span class="--text"><xsl:value-of select="."/></span><span class="markup">&lt;/</span><span class="nodeName element-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/></span><span class="markup">></span>
		</div>
	</xsl:template>

	<xsl:template match="@*" mode="tree">
		<xsl:text> </xsl:text><span class="attrName attribute-{translate(name(), ':', '_')}"><xsl:value-of select="name()"/></span><span class="markup">="</span><span class="attrValue"><xsl:value-of select="."/></span><span class="markup">"</span></xsl:template>
	...
	<xsl:template match="comment()" mode="tree">
		...
		<div class="comment">
			...
			<span class="markup">&lt;&#33;&#45;&#45;</span>
			<div class="commentText"><pre><xsl:value-of select="."/></pre></div>
			<span class="switch_space">&#160;</span>
			<span class="markup">&#45;&#45;&gt;</span>
		</div>
	</xsl:template>
	...
	<xsl:template match="processing-instruction('xml')" mode="tree">
		<div class="element">
			<span class="switch_space">&#160;</span>
			<span class="markup">&lt;?</span>
			<span class="pi"><xsl:text>xml </xsl:text>
				<xsl:for-each select="@*">
					<xsl:value-of select="name(.)"/>
					<xsl:text>="</xsl:text>
					<xsl:value-of select="."/>
	...
	<xsl:template match="@href" mode="tree">
		<xsl:text> </xsl:text>
		<span class="attrName attribute-href"><xsl:value-of select="name(.)"/></span>
		<span class="markup">="</span>
		<span class="attrValue">
			<xsl:choose>
